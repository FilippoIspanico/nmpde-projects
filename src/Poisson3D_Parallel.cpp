...
void
Poisson3DParallel::write_csv(const long int elapsed_time, int iterations, double cond_number, TrilinosWrappers::PreconditionAMG::size_type memoryUsage) const{
...
Poisson3DParallel::solve()
{
  ...
  TrilinosWrappers::PreconditionAMG::size_type memoryUsage;
  ...
  const auto t0 = std::chrono::high_resolution_clock::now();
    ...
    if (preconditioner_name == "identity")
    {
        ...
        memoryUsage = preconditioner.memory_consumption();
  ...
  if (mpi_rank == 0)
      write_csv(dt, solver_control.last_step(), condition_number, memoryUsage);
